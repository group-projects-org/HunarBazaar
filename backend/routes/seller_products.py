...
async def add_product(request: Request, name: str = Form(...), price: float = Form(...), category: str = Form(...), description: str = Form(...), variants: str = Form(...), tags: Optional[str] = Form(None), images: Optional[List[UploadFile]] = File(None), image_links: Optional[List[str]] = Form(None)):
    try:
        payload = get_current_user(request)
        seller_id = payload.get("user_id")
        ...
        if image_links: image_urls.extend(image_links)
